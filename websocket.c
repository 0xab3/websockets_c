...
internal void ws_read(Ws_Context *ctx) {
  uint8_t header[WS_MAX_HEADER_LENGTH] = {0};
  io_send_read_event(&ctx->io_ctx, ctx->tcp_fd, header, WS_MAX_HEADER_LENGTH);
}
...
WS_STATUS ws_connect(Ws_Context *ctx) {
  ...
  int io_read_event_send_status =
      io_send_read_event(&ctx->io_ctx, ctx->tcp_fd, buffer, 1024);
  assert(io_read_event_send_status == 1);
  const ssize_t io_event_read_size =
      io_get_processed_event_result(&ctx->io_ctx);
